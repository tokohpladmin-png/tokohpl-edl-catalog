...
import { getNewCollectionProducts, getPromoItemProducts } from '@/lib/products';
  ...
  return (
    <div>
      <section className="relative isolate overflow-hidden bg-[#ece7dd] text-[#241f1a]">
        ...
        <div className="absolute left-[-12rem] top-[-10rem] -z-10 h-[32rem] w-[32rem] rounded-none bg-white/55 blur-3xl" />
        ...
        <div className="section-shell grid min-h-[calc(100vh-74px)] items-center gap-12 py-16 sm:py-20 lg:grid-cols-[1.05fr_0.95fr] lg:py-24">
          <div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link href="/products" className="inline-flex items-center justify-center rounded-none bg-[#241f1a] px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-[#3a332c]">
                Browse Collection
              </Link>
              <Link href="/contact" className="inline-flex items-center justify-center rounded-none border border-[#241f1a]/20 bg-white/70 px-5 py-3 text-sm font-bold text-[#241f1a] transition hover:-translate-y-0.5 hover:bg-white">
                Contact TokoHPL
              </Link>
            </div>
          </div>
          ...
          <div className="soft-card relative z-10 overflow-hidden !border-white/30 !bg-white/55 p-4 shadow-luxury backdrop-blur">
            <div className="rounded-none bg-white p-5 text-[#241f1a] sm:p-6">
              <div className="rounded-none bg-[#f4f0e8] p-4">
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#8a4f2b]">Search Products</p>
                <form action="/products" className="relative z-20 mt-4 flex rounded-none border border-[#d8cbb9] bg-white shadow-sm">
                  <input
                    type="search"
                    name="search"
                    aria-label="Search products"
                    placeholder="Search EDL product code, color, or surface"
                    className="min-w-0 flex-1 rounded-none bg-transparent px-5 py-4 text-sm font-medium text-[#241f1a] outline-none placeholder:text-stone-400"
                  />
                  <button type="submit" aria-label="Search" className="m-1 flex h-11 w-11 shrink-0 items-center justify-center rounded-none bg-[#241f1a] text-white transition hover:bg-[#3a332c]">
                    →
                  </button>
                </form>
              </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {collectionCards.map((card) => (
                  <Link key={card.href} href={card.href} className="rounded-none border border-[#e4dbce] bg-white p-4 transition hover:-translate-y-0.5 hover:shadow-sm">
...
<section className="section-shell py-14 sm:py-20">
  <div className="mb-8 max-w-3xl">
    <p className="eyebrow">Why Shop with TokoHPL</p>
    <h2 className="mt-3 text-3xl font-black tracking-[-0.04em] text-stone-950 sm:text-5xl">
      A smoother way to source EDL decorative surfaces.
    </h2>
    <p className="mt-4 text-sm leading-7 text-stone-600 sm:text-base">
      TokoHPL is built to make product discovery, enquiry, and ordering clearer for contractors, interior businesses, and project teams.
    </p>
  </div>
    ...
    {[
      {
        ...
        description: 'Source EDL products through a trusted channel with clear product information and support.'
      },
      {
        icon: '＋',
        title: 'Easy Ordering',
        description: 'Browse products, add selections to cart, and submit your enquiry in a simple online flow.'
      },
      {
        icon: '▣',
        title: 'Free Delivery',
        description: 'Enjoy free delivery coverage for eligible orders across Java and Bali.'
      },
      {
        icon: '☺',
        title: 'Customer-Focused Service',
        description: 'Our team helps with product questions, stock checks, order details, and delivery guidance.'
      },
      {
        icon: '🔒',
        title: 'Secure & Transparent',
        description: 'Order details, pricing, and availability are confirmed before your purchase is finalised.'
      }
    ].map((item) => (
      <article key={item.title} className="rounded-none border border-stone-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-card">
        <div className="flex h-14 w-14 items-center justify-center rounded-none bg-[#241f1a] text-2xl font-black text-white">
          {item.icon}
        </div>
        <h3 className="mt-6 text-lg font-black tracking-[-0.025em] text-stone-950">{item.title}</h3>
    ...
    <div className="grid gap-0 lg:grid-cols-[0.92fr_1.08fr]">
      <div className="bg-[#241f1a] p-8 text-white sm:p-10 lg:p-12">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-7 inline-flex rounded-none bg-white px-6 py-3 text-sm font-black text-[#241f1a] transition hover:bg-stone-100"
        >
          Explore
        </a>
      </div>
