...
export function ProductCard({ product, showPromoPricing = false }: { product: Product; showPromoPricing?: boolean }) {
  return (
    <article className="group overflow-hidden border border-stone-200/80 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-card">
      <Link href={`/products/${product.slug}`} className="block" aria-label={`View ${product.name}`}>
        <div data-product-thumbnail="true" className="relative aspect-square overflow-hidden bg-gradient-to-br from-stone-100 to-stone-50">
        ...
        <div className="mt-3">
          {showPromoPricing && product.isPromoItem && typeof product.promoPrice === 'number' ? (
            <>
              ...
              <p className="mt-1 inline-flex bg-amber-100 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-amber-900">
                Promo 5% Off
              </p>
            </>
          ) : (
          ...
          <p className="mt-1 text-[11px] font-bold uppercase tracking-[0.12em] text-stone-500">
            Tax included
          </p>
        </div>
      </div>
